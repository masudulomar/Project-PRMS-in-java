...
public class LoginValidation {
	...
	public Map<String, String> UserCreation(Map DataMap) {
		...
		try {
			...
			rs = stmt.executeQuery();
			while (rs.next()) {
				String SQL = "UPDATE PRMS_USER U SET U.USER_PASSWORD = ? WHERE U.USER_ID= ?";
			...
			ResultMap.put("ERROR_MSG", "Error in Updating Password");
			e.printStackTrace();
		} finally {
			try {
				con.close();
	 ...
	 * 
	 * DBUtils ob = new DBUtils(); String sql =
	 * "select * from PRMS_USER t where t.user_id=? and "; Connection con =
	...
	public Map<String, String> LoginMethod(Map DataMap) {
		...
		ResultSet rs = null;
		String sql = "SELECT T.USER_ID, T.USER_NAME, T.USER_BRANCH, T.USER_ROLE, (SELECT M.BRN_NAME FROM PRMS_MBRANCH M WHERE M.BRN_CODE = T.USER_BRANCH) BRN_NAME FROM PRMS_USER T WHERE T.USER_ID=? AND T.USER_PASSWORD=?";
		PreparedStatement stmt = null;
		try {
			stmt = con.prepareStatement(sql);
			stmt.setString(1, DataMap.get("user").toString());
